...
# Усиленный промпт для короткого видео-текста
SYSTEM_PROMPT_VOICE = """
Ты — голос сервиса AvtoMaster24. Задача: создать короткий текст (30–50 слов) для озвучки видео. 
Текст должен быть связным, законченным, содержать:
- Название услуги.
- Проблему, которую она решает (одна короткая фраза).
- Призыв: «Найди проверенных исполнителей в боте AvtoMaster24 и закажи помощь за минуту».
Не используй имя «Тимур», говори от лица сервиса.
Пример: «Нужно открыть авто без ключа? AvtoMaster24 поможет найти мастера рядом. Переходи в бот и решай проблему за минуту!»
"""
...
async def generate_voice_text(service):
    """Генерирует короткий текст (30–50 слов) для видео, с контролем длины."""
    prompt = f"""
    {SYSTEM_PROMPT_VOICE}
    Услуга: **{service}**.
    Создай короткий, законченный текст для озвучки видео (30–50 слов). 
    Обязательно начни с вопроса или утверждения, затем кратко опиши проблему, которую решает услуга, и закончи призывом перейти в бот AvtoMaster24.
    Не используй имя "Тимур", говори от лица сервиса.
    """
    model_name = get_available_model()
        ...
        try:
            ...
            if raw_text:
                ...
                if raw_text.endswith('…'):
                    raw_text = raw_text[:-1] + '.'
                if len(raw_text) < 30:
                    logger.warning(f"Сгенерированный текст слишком короткий ({len(raw_text)} символов). Повторяем...")
                    continue
                return raw_text
        except Exception as e:
